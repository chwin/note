...
`` # 来回跳转
`. # 不仅跳到最后修改的那一行，还要定位到修改点 g; 或、`. # 不仅跳到最后修改的那一行，还要定位到修改点
`# 跳转到某标签的光标位置 ' # 跳转到某标签的行首 ma # 在当前光标位置设置位置 a 标记（可使用范围'a-za-z')`a # 跳转到 a 标记
C-o # 跳回上一次的 jump
C-i # 跳回下一次的 jump
...
:%s/^\\n#g 删除空行
:%s/^ ##g 删除行首的空格
:%s/ #$#g 删除行尾的空格 :%s/^\n\{3}# 可以用以下命令删除三行空行： :%s/\n\n/\r/g 可以用以下命令将连续的两个空行替换成一个空行 :0,$

KaTeX parse error: Expected 'EOF', got '#' at position 1: #̲g 删除行尾的空格 :%s/^…

KaTeX parse error: Expected 'EOF', got '#' at position 1: #̲g 删除行尾的空格 :%s/^…

KaTeX parse error: Expected 'EOF', got '#' at position 1: #̲g 删除行尾的空格 :%s/^…

KaTeX parse error: Expected 'EOF', got '#' at position 1: #̲g 删除行尾的…

s/^/#/gc 在行首加一个#号
:6,10s/^/#/gc 在 6~10 行的行首加一个#号
:%s= #$== 将所有行尾多余的空格删除 :g/^s#$

KaTeX parse error: Expected 'EOF', got '#' at position 22: …行尾多余的空格删除 :g/^s#̲

KaTeX parse error: Expected 'EOF', got '#' at position 22: …行尾多余的空格删除 :g/^s#̲

KaTeX parse error: Expected 'EOF', got '#' at position 22: …行尾多余的空格删除 :g/^s#̲

KaTeX parse error: Expected 'EOF', got '#' at position 31: …行尾多余的空格删除 :g/^s#̲

/d 将所有不包含字符（空格也不包含）的空行删除。
:s# 和：g#，:!g#
...
:m,nw >> <file>将 m 行到 n 行的内容添加到文件 <file>的末尾</file></file>
